...
YELLOW_SPACE_SHIP = pygame.transform.scale(pygame.image.load('pixel_ship_yellow.png'),(50,50))
...
class Player(Ship):
			...
			if laser.off_screen(HEIGHT):
				...
				for obj in objs:
					if laser.collision(obj):
						objs.remove(obj)
						self.score = scoreup(self.score)
						self.draw_score(WIN,self.score)
	...
	def draw_score(self,window,score):
		...
		window.blit(score_label,(WIDTH//2-40,10))
...
class Enemy(Ship):
	...
	def shoot(self):
		if self.cool_down_counter == 0:
			if self.ship_img.get_width() ==50:
				laser = Laser(self.x-self.ship_img.get_width()//2,self.y,self.laser_img)
			else:
				laser = Laser(self.x-self.ship_img.get_width()//2+20,self.y,self.laser_img)
			self.lasers.append(laser)
			self.cool_down_counter = 1
...
def main():
	...
	def redraw_win():
		...
		player.draw_score(WIN,player.score)
